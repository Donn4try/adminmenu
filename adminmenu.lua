...
script_description('This is a Cross-platform Lua script helper for Grand Country RP admins')
script_author("4trays")
script_version("3.8")
...
print('[Admin Helper] Skrip telah diinisialisasi. Versi: ' .. thisScript().version)
-------------------------------------------- JSON SETTINGS ---------------------------------------------
local configDirectory = getWorkingDirectory():gsub('\\','/') .. "/admin Helper"
local path_helper = getWorkingDirectory():gsub('\\','/') .. "/admin Helper.lua"
...
local default_settings = {
	...
	player_info = {
		name_surname = 'DonJuan',
		accent = '',
		fraction = 'Administrator',
		fraction_tag = 'Faction Section',
		fraction_rank = 'Volunteer',
		fraction_rank_number = 4,
		sex = 'Tidak Diketahui',
	},
	deportament = {
		dep_fm = '-',
		dep_tag1 = '',
		dep_tag2 = '[Untuk semua]',
		dep_tags = {
			"[Untuk semua]",
			"[Похитители]",
			"[Терористы]",
...
function load_settings()
    ...
    if not doesFileExist(path_settings) then
		...
		print('[Admin Helper] File pengaturan tidak ditemukan, menggunakan pengaturan standar!')
    else
        ...
        if file then
			...
			if #contents == 0 then
				settings = default_settings
				print('[Admin Helper] Gagal membaca file pengaturan, menggunakan pengaturan standar!')
			else
				...
				if result then
					...
					if settings.general.version ~= thisScript().version then
						print('[Admin Helper] Versi baru terdeteksi, reset pengaturan!')
						settings = default_settings
						save_settings()
						reload_script = true
					else
						print('[Admin Helper] Pengaturan berhasil dimuat!')
					end
				else
					print('[Admin Helper] Gagal membuka file pengaturan, menggunakan pengaturan standar!')
				end
			end
        else
            settings = default_settings
...
function save_settings()
    ...
    if file then
        ...
        file:close()
		print('[Admin Helper] Pengaturan berhasil disimpan!')
        return result
    else
        print('[Admin Helper] Gagal menyimpan pengaturan, error: ', errstr)
        return false
    end
end
load_settings()
...
local notes = {
	note = {
		{ note_name = 'Punishment', note_text = 'JAIL PUNISHMENT'},
	}
}
...
function load_notes()
	if doesFileExist(path_notes) then
        ...
        if file then
            ...
            file:close()
			if #contents == 0 then
				print('[Admin Helper] Gagal memuat catatan!')
				print('[Admin Helper] Penyebab: file kosong')
			else
				local result, loaded = pcall(decodeJson, contents)
				if result then
					notes = loaded
					print('[Admin Helper] Catatan berhasil dimuat!')
				else
					print('[Admin Helper] Gagal memuat catatan!')
					print('[Admin Helper] Penyebab: file JSON rusak')
				end
			end
        else
			print('[Admin Helper] Gagal membuka file catatan!')
			print('[Admin Helper] Menyebabkan: ')
        end
	else
		print('[Admin Helper] Gagal membuka berkas dengan catatan!')
		print('[Admin Helper] File catatan tidak ditemukan di '..configDirectory)
	end
end
function save_notes()
    ...
    if file then
        ...
        file:write(result and encoded or "")
        file:close()
		print('[Admin Helper] Catatan berhasil disimpan!')
        return result
    else
        print('[Admin Helper] Gagal menyimpan catatan, error: ', errstr)
        return false
    end
end
load_notes()
...
local commands = {
	commands = {
		{ cmd = 'ps' , description = 'Player Statistic' , text = '/astats {arg_id}', arg = '{arg_id}' , enable = true , waiting = '1.200', deleted = false },
		{ cmd = 'plh' , description = 'Player Health' , text = '/ahealth {arg_id}', arg = '{arg_id}' , enable = true, waiting = '1.200', deleted = false   },
		{ cmd = 'pd' , description = 'Player Damagelogs' ,  text = '/adamagelogs {arg_id}', arg = '{arg_id}' , enable = true, waiting = '1.200', deleted = false   },
		{ cmd = 'fp' , description = 'Freeze Player' , text = '/freeze {arg_id}', arg = '{arg_id}' , enable = true, waiting = '1.200', deleted = false   },
		{ cmd = 'up' , description = 'Unfreeze Player' ,  text = '/unfreeze {arg_id}' , arg = '{arg_id}' , arg = '{arg_id}' , enable = true , waiting = '1.500', deleted = false  },		
		{ cmd = 'slop' , description = 'Slap Player' ,  text = '/slap {arg_id}' , arg = '{arg_id}', enable = true, waiting = '1.200', deleted = false} , 
		{ cmd = 'rip' , description = 'Revive Player' ,  text = '/arv {arg_id}' , arg = '{arg_id}' , enable = true , waiting = '1.200' , deleted = false },
		
	},
	commands_manage = {
		{ cmd = 'sn' , description = 'Set player skin', text = '/setskin {arg_id}' , arg = '{arg_id} {arg2}', enable = true, waiting = '1.200', deleted = false  },	
	}
}
...
function load_commands()
	if doesFileExist(path_commands) then
        ...
        if file then
            ...
            file:close()
			if #contents == 0 then
				print('[Admin Helper] Gagal membuka berkas perintah!')
				print('[Admin Helper] Alasan: File ini kosong')
			else
				local result, loaded = pcall(decodeJson, contents)
				if result then
					if tostring(settings.general.version) ~= tostring(thisScript().version) then 
						print('[Admin Helper] Versi lain ditemukan, mencoba menambahkan perintah baru')
						...
						for category, _ in pairs(commands) do
							...
							for key, value in pairs(commands[category]) do
								if temp_commands[category][key] == nil then
									...
									print('[Admin Helper] Menambahkan perintah /' .. value.cmd)
								end
							end
						end
						save_commands()
						...
						for _, command in pairs(loaded.commands) do
							if not command.bind then
								print('[Admin Helper] Memperbarui tim /' .. command.cmd .. ' (dukungan untuk ikatan)')
								command.bind = "{}"
							end
						end
					...
					print('[Admin Helper] Semua perintah diinisialisasi!')
					
				else
					print('[Admin Helper] Gagal membuka berkas perintah!')
					print('[Admin Helper] Alasan: Gagal mendekode json (kesalahan dalam file)')
				end
			end
        else
			print('[Admin Helper] Gagal membuka berkas perintah!')
			print('[Admin Helper] Menyebabkan: ', errstr)
        end
	else
		print('[Admin Helper] Gagal membuka berkas perintah!')
		print('[Admin Helper] Alasan: File ini tidak ada di folder '..configDirectory)
		print('[Admin Helper] Inisialisasi perintah standar...')
		save_commands()
		load_commands()
	end
end
function save_commands()
    ...
    if file then
        ...
        file:write(result and encoded or "")
        file:close()
		print('[Admin Helper] Perintah Anda telah disimpan!')
        return result
    else
        print('[Admin Helper] Gagal menyimpan perintah pembantu, kesalahan: ', errstr)
        return false
    end
end
load_commands()
...
if not settings.general.autofind_dpi then
	print('[Admin Helper] Применение авто-размера менюшек...')
	if isMonetLoader() then
	...
	print('[Admin Helper] Установлено значение: ' .. settings.general.custom_dpi)
	save_settings()
end
---------------------------------------------- Mimgui -----------------------------------------------------
...
local input_accent = imgui.new.char[256]((settings.player_info.accent))
local input_name_surname = imgui.new.char[256]((settings.player_info.name_surname))
local input_fraction_tag = imgui.new.char[256]((settings.player_info.fraction_tag))
...
local input_dep_fm = imgui.new.char[32]((settings.deportament.dep_fm))
local input_dep_text = imgui.new.char[256]()
local input_dep_tag1 = imgui.new.char[32]((settings.deportament.dep_tag1))
local input_dep_tag2 = imgui.new.char[32]((settings.deportament.dep_tag2))
...
local item_list = {'Tidak ada argumen', '{arg} - menerima argumen apa pun', '{arg_id} - hanya menerima argumen ID pemain', '{arg_id} {arg2} - menerima 2 argumen: ID pemain dan argumen apa pun', '{arg_id} {arg2} {'}
...
local tagReplacements = {
	...
	sex = function() 
		if settings.player_info.sex == 'Wanita' then
			local temp = 'а'
			return temp
		else
			return ''
...
{my_id} - ID Anda
{my_nick} - Nama Panggilan Anda 
{my_rp_nick} - Nama Panggilan Anda tanpa _
{my_ru_nick} - Username
{my_doklad_nick} - Huruf pertama nama depan dan nama belakang Anda

{fraction} - Fraksi Anda
{fraction_rank} - Posisi faksi Anda
{fraction_tag} - Tag faksi Anda

{sex} - Menambahkan huruf "a" jika helper menentukan jenis kelamin perempuan

{get_time} - Dapatkan waktu saat ini
{get_city} - Dapatkan kota saat ini
{get_nick({arg_id})} - dapatkan Nama Panggilan dari argumen ID pemain
{get_rp_nick({arg_id})} - dapatkan Nama Panggilan tanpa simbol _ dari argumen ID pemain
{get_ru_nick({arg_id})} - dapatkan Nama Panggilan dalam Cyrillic dari argumen ID pemain 

{pause} - Hentikan permainan tim dan tunggu]]

-------------------------------------------- MoonMonet ----------------------------------------------------

local monet_no_errors, moon_monet = pcall(require, 'MoonMonet') -- menghubungkan perpustakaan dengan aman
...
if not isMonetLoader() then
	...
	if hotkey_no_errors then
		hotkey.Text.NoKey = '< click and select keys >'
		hotkey.Text.WaitForKey = '< wait keys >'
		...
		function updateHotkeyForCommand(command)
			...
			if command.arg == '' and command.bind ~= nil and command.bind ~= '{}' and command.bind ~= '[]' then
				...
				print('[Admin Helper] Membuat hotkey untuk perintah /'.. command.cmd..' pada tombol ' .. getNameKeysFrom(command.bind))
				--sampAddChatMessage('[Admin Helper] {ffffff}Tombol pintas untuk perintah telah dibuat ' .. message_color_hex .. '/' .. command.cmd .. ' {ffffff}pada kunci '  .. message_color_hex .. getNameKeysFrom(command.bind), message_color)
			end
		end
...
function welcome_message()
	if not sampIsLocalPlayerSpawned() then 
		sampAddChatMessage('[Admin Helper] {ffffff}Inisialisasi helper berhasil!',message_color)
		sampAddChatMessage('[Admin Helper] {ffffff}Untuk memuat helper sepenuhnya, spawn terlebih dahulu (masuk ke server)',message_color)
		repeat wait(0) until sampIsLocalPlayerSpawned()
	end
	sampAddChatMessage('[Admin Helper] {ffffff}Pemuatan helper berhasil diselesaikan!', message_color)
	print('[Admin Helper] Pemuatan pembantu berhasil diselesaikan!')
	show_arz_notify('info', 'Admin Helper', "Pemuatan pembantu berhasil diselesaikan!", 3000)
	if isMonetLoader() or settings.general.bind_mainmenu == nil or not settings.general.use_binds then	
		sampAddChatMessage('[Admin Helper] {ffffff}Untuk membuka menu helper, masukkan command ' .. message_color_hex .. '/ahmenu', message_color)
	elseif hotkey_no_errors and settings.general.bind_mainmenu and settings.general.use_binds then
		sampAddChatMessage('[Admin Helper] {ffffff} Untuk membuka menu helper, klik ' .. message_color_hex .. getNameKeysFrom(settings.general.bind_mainmenu) .. ' {ffffff}atau masukkan command ' .. message_color_hex .. '/ahmenu', message_color)
	else
		sampAddChatMessage('[Admin Helper] {ffffff}Untuk membuka menu helper, masukkan command ' .. message_color_hex .. '/ahmenu', message_color)
	end
end
		...
		if not isActiveCommand then
			...
			if cmd_arg == '{arg}' then
				if arg and arg ~= '' then
					...
					sampAddChatMessage('[Admin Helper] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [аргумент]', message_color)
					play_error_sound()
				end
			elseif cmd_arg == '{arg_id}' then
				if isParamSampID(arg) then
					...
					sampAddChatMessage('[Admin Helper] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока]', message_color)
					play_error_sound()
				end
			elseif cmd_arg == '{arg_id} {arg2}' then
				if arg and arg ~= '' then
					...
					if isParamSampID(arg_id) and arg2 then
						...
						sampAddChatMessage('[Admin Helper] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока] [аргумент]', message_color)
						play_error_sound()
					end
				else
					sampAddChatMessage('[Admin Helper] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока] [аргумент]', message_color)
            ...
            elseif cmd_arg == '{arg_id} {arg2} {arg3}' then
				if arg and arg ~= '' then
					...
					if isParamSampID(arg_id) and arg2 and arg3 then
						...
						sampAddChatMessage('[Admin Helper] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока] [аргумент] [аргумент]', message_color)
						play_error_sound()
					end
				else
					sampAddChatMessage('[Admin Helper] {ffffff}Используйте ' .. message_color_hex .. '/' .. chat_cmd .. ' [ID игрока] [аргумент] [аргумент]', message_color)
				...
				lua_thread.create(function()
					...
					if modifiedText:find('&.+&') then
						if isMonetLoader() and settings.general.mobile_stop_button then
							sampAddChatMessage('[Admin Helper] {ffffff}Чтобы остановить отыгровку команды используйте ' .. message_color_hex .. '/stop {ffffff}или нажмите кнопку внизу экрана', message_color)
						...
						elseif not isMonetLoader() and hotkey_no_errors and settings.general.bind_command_stop and settings.general.use_binds then
							sampAddChatMessage('[Admin Helper] {ffffff}Чтобы остановить отыгровку команды используйте ' .. message_color_hex .. '/stop {ffffff}или нажмите ' .. message_color_hex .. getNameKeysFrom(settings.general.bind_command_stop), message_color)
						else
							sampAddChatMessage('[Admin Helper] {ffffff}Чтобы остановить отыгровку команды используйте ' .. message_color_hex .. '/stop', message_color)
						end
					end
					local lines = {}
					...
					for line_index, line in ipairs(lines) do
						if command_stop then 
							...
							sampAddChatMessage('[Admin Helper] {ffffff}Отыгровка команды /' .. chat_cmd .. " успешно остановлена!", message_color) 
							break	
						else
							...
							elseif line == "{pause}" then
								sampAddChatMessage('[Admin Helper] {ffffff}Perintah /'..chat_cmd..' dihentikan sementara!', message_color)
								command_pause = true
								...
								if not command_stop then
									sampAddChatMessage('[Admin Helper] {ffffff}Продолжаю отыгровку команды /' .. chat_cmd, message_color)	
								end					
							else
			...
			sampAddChatMessage('[Admin Helper] {ffffff}Дождитесь завершения отыгровки предыдущей команды!', message_color)
			play_error_sound()
		end
	end)
end
function find_and_use_command(cmd, cmd_arg)
	...
	if not check then
		sampAddChatMessage('[Admin Helper] {ffffff}Ошибка, не могу найти бинд для выполнения этой команды!', message_color)
		play_error_sound()
		return
	end
end
function initialize_commands()
	sampRegisterChatCommand("ahmenu", function() MainWindow[0] = not MainWindow[0] end)
	sampRegisterChatCommand("stop", function() 
		if isActiveCommand then 
			command_stop = true 
		else 
			sampAddChatMessage('[Admin Helper] {ffffff}В данный момент нету никакой активной команды/отыгровки!', message_color) 
		end
	end)
	sampRegisterChatCommand("sob", function(arg)
		if not isActiveCommand then
			if isParamSampID(arg) then
				...
				sampAddChatMessage('[Admin Helper] {ffffff}Используйте ' .. message_color_hex .. '/sob [ID игрока]', message_color)
				play_error_sound()
			end	
		else
			sampAddChatMessage('[Admin Helper] {ffffff}Дождитесь завершения отыгровки предыдущей команды!', message_color)
	...
	sampRegisterChatCommand("debug", function() debug_mode = not debug_mode sampAddChatMessage('[GOV DEBUG] {ffffff}Отслеживание команд ' .. (debug_mode and 'включено!' or 'выключено!'), message_color) end)
			...
			if not isActiveCommand then
				lua_thread.create(function()
					...
					if command_stop then 
						...
						sampAddChatMessage('[Admin Helper] {ffffff}Отыгровка команды /spcar успешно остановлена!', message_color) 
						return
					end
...
function play_error_sound()
	...
	show_arz_notify('error', 'Admin Helper', "Произошла ошибка!", 1500)
end
function show_leader_fast_menu(id)
	if isParamSampID(id) then
		...
		if isMonetLoader() or settings.general.bind_leader_fastmenu == nil then
			sampAddChatMessage('[Admin Helper] {ffffff}Используйте ' .. message_color_hex .. '/glm [ID]', message_color)
		elseif settings.general.bind_leader_fastmenu and settings.general.use_binds and hotkey_no_errors then
			sampAddChatMessage('[Admin Helper] {ffffff}Используйте ' .. message_color_hex .. '/glm [ID] {ffffff}или наведитесь на игрока через ' .. message_color_hex .. 'ПКМ + ' .. getNameKeysFrom(settings.general.bind_leader_fastmenu), message_color) 
		else
			sampAddChatMessage('[Admin Helper] {ffffff}Используйте ' .. message_color_hex .. '/glm [ID]', message_color)
		end 
		play_error_sound()
...
function ifCommandPause()
	if command_stop then 
		...
		sampAddChatMessage('[Admin Helper] {ffffff}Отыгровка команды успешно остановлена!', message_color)  
		return true
	else
		return false
	end
	...
	if isParamSampID(id) then 
		...
		if isMonetLoader() or settings.general.bind_fastmenu == nil then
			if not FastMenuPlayers[0] then
				sampAddChatMessage('[Admin Helper] {ffffff}Используйте ' .. message_color_hex .. '/gm [ID]', message_color)
			end
		elseif settings.general.bind_fastmenu and settings.general.use_binds and hotkey_no_errors then
			sampAddChatMessage('[Admin Helper] {ffffff}Используйте ' .. message_color_hex .. '/gm [ID] {ffffff}или наведитесь на игрока через ' .. message_color_hex .. 'ПКМ + ' .. getNameKeysFrom(settings.general.bind_fastmenu), message_color) 
		else
			sampAddChatMessage('[Admin Helper] {ffffff}Используйте ' .. message_color_hex .. '/gm [ID]', message_color)
		end 
	end 
...
function sampev.onServerMessage(color,text)
	...
	if (settings.general.auto_uval and tonumber(settings.player_info.fraction_rank_number) >= 9) then
		...
		if text:find("(.+) заглушил%(а%) игрока (.+) на 1 минут. Причина: %[AutoUval%] Ожидайте") and auto_uval_checker then
			...
			lua_thread.create(function ()
				wait(50)
				if Name == MyName then
					sampAddChatMessage('[Admin Helper] {ffffff}Увольняю игрока ' .. sampGetPlayerNickname(PlayerID) .. '!', message_color)
					auto_uval_checker = false
					temp = PlayerID .. ' ПСЖ'
					find_and_use_command("/uninvite {arg_id} {arg2}", temp)
				else
					sampAddChatMessage('[Admin Helper] {ffffff}Другой заместитель/лидер уже увольняет игрока ' .. sampGetPlayerNickname(PlayerID) .. '!', message_color)
					auto_uval_checker = false
				end
			end)
		end
	...
	if (text:find("У (.+) отсутствует трудовая книжка. Вы можете выдать ему книжку с помощью команды /givewbook") and tonumber(settings.player_info.fraction_rank_number) >= 9) then
		...
		sampAddChatMessage('[Admin Helper] {ffffff}У игрока ' .. nick .. ' нету трудовой книжки, выдайте её используя ' .. message_color_hex .. cmd .. ' ' .. sampGetPlayerIdByNickname(nick), message_color)
		return false
	end
	...
	if text:find("Номера телефонов государственных служб:") then
		sampAddChatMessage('[Admin Helper] {ffffff}Номера телефонов государственных служб:', message_color)
		sampAddChatMessage('[Admin Helper] {ffffff}111 Баланс | 60 Время | 911 МЮ | 912 МЗ | 913 Такси | 914 Мехи | 8828 Банк | 997 Дома', message_color)
		return false
	end
...
function sampev.onSendChat(text)
	if debug_mode then
		sampAddChatMessage('[GOV DEBUG] {ffffff}' .. text, message_color)
	end
	local ignore = {
		[";)"] = true,
		[":D"] = true,
...
function sampev.onShowDialog(dialogid, style, title, button1, button2, text)
	...
	if title:find('Основная статистика') and check_stats then -- получение статистики
		if text:find("{FFFFFF}Имя: {B83434}%[(.-)]") then
			...
			sampAddChatMessage('[Admin Helper] {ffffff}Ваше Имя и Фамилия обнаружены, вы - ' .. settings.player_info.name_surname, message_color)
		end
		if text:find("{FFFFFF}Пол: {B83434}%[(.-)]") then
			settings.player_info.sex = text:match("{FFFFFF}Пол: {B83434}%[(.-)]")
			sampAddChatMessage('[Admin Helper] {ffffff}Ваш пол обнаружен, вы - ' .. settings.player_info.sex, message_color)
		end
		if text:find("{FFFFFF}Организация: {B83434}%[(.-)]") then
			...
			if settings.player_info.fraction == 'Не имеется' then
				sampAddChatMessage('[Admin Helper] {ffffff}Вы не состоите в организации!',message_color)
				settings.player_info.fraction_tag = "Неизвестно"
			else
				sampAddChatMessage('[Admin Helper] {ffffff}Ваша организация обнаружена, это: '..settings.player_info.fraction, message_color)
				...
				sampAddChatMessage('[Admin Helper] {ffffff}Вашей организации присвоен тег '..settings.player_info.fraction_tag .. ". Но вы можете изменить его.", message_color)
			end
		end
		if text:find("{FFFFFF}Должность: {B83434}(.+)%((%d+)%)") then
			...
			sampAddChatMessage('[Admin Helper] {ffffff}Ваша должность обнаружена, это: '..settings.player_info.fraction_rank.." ("..settings.player_info.fraction_rank_number..")", message_color)
			if tonumber(settings.player_info.fraction_rank_number) >= 9 then
			...
			sampAddChatMessage('[Admin Helper] {ffffff}Произошла ошибка, не могу получить ваш ранг!',message_color)
		end
		save_settings()
	...
	if members_check and title:find('(.+)%(В сети: (%d+)%)') then -- мемберс 
		...
		elseif #members_new ~= 0 then
			...
			sampSendDialogResponse(dialogid, 0, 0, 0)
			sampAddChatMessage('[Admin Helper]{ffffff} Список сотрудников пуст!', message_color)
			members_check = false
        end
        return false
    end
...
function give_rank()
	local command_find = false
					...
					lua_thread.create(function()
						...
						for _, line in ipairs(lines) do 
							if command_stop then 
								...
								sampAddChatMessage('[Admin Helper] {ffffff}Отыгровка команды /' .. command.cmd .. " успешно остановлена!", message_color) 
								return 
							end
							if wait_tag then
								for tag, replacement in pairs(tagReplacements) do
			...
			if not command_find then
				sampAddChatMessage('[Admin Helper] {ffffff}Бинд для изменения ранга отсутствует либо отключён!', message_color)
				sampAddChatMessage('[Admin Helper] {ffffff}Попробуйте сбросить настройки хелпера!', message_color)
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(fa.ROBOT .. " Admin Helper##main", MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize )
		change_dpi()
		if imgui.BeginTabBar('пон') then	
			if imgui.BeginTabItem(fa.ID_CARD..' Menu utama') then
				if imgui.BeginChild('##1', imgui.ImVec2(589 * settings.general.custom_dpi, 171 * settings.general.custom_dpi), true) then
					imgui.CenterText(fa.USER_NURSE .. ' Informasi administrator')
					imgui.Separator()
					imgui.Columns(3)
					imgui.CenterColumnText("Nama:")
					imgui.SetColumnWidth(-1, 230 * settings.general.custom_dpi)
					imgui.NextColumn()
					imgui.CenterColumnText((settings.player_info.name_surname))
					imgui.SetColumnWidth(-1, 250 * settings.general.custom_dpi)
					imgui.NextColumn()
					if imgui.CenterColumnSmallButton('Mengubah##name_surname') then
						settings.player_info.name_surname = TranslateNick(sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))))
						input_name_surname = imgui.new.char[256](u8(settings.player_info.name_surname))
						save_settings()
						imgui.OpenPopup(fa.USER_NURSE .. ' Nama Depan dan Belakang##name_surname')
					end
					if imgui.BeginPopupModal(fa.USER_NURSE .. ' Nama Depan dan Belakang##name_surname', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize ) then
						...
						imgui.InputText('##name_surname', input_name_surname, 256) 
						imgui.Separator()
						if imgui.Button(fa.CIRCLE_XMARK .. ' Membatalkan', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
							imgui.CloseCurrentPopup()
						end
						imgui.SameLine()
						if imgui.Button(fa.FLOPPY_DISK .. ' Menyimpan', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
					...
					imgui.Separator()
					imgui.Columns(3)
					imgui.CenterColumnText("Gender:")
					imgui.NextColumn()
					imgui.CenterColumnText((settings.player_info.sex))
					imgui.NextColumn()
					if imgui.CenterColumnSmallButton('Mengubah##sex') then
						if settings.player_info.sex == 'Tidak dikenal' then
							settings.player_info.sex = 'Wanita'
							save_settings()
						elseif settings.player_info.sex == 'Pria' then
							settings.player_info.sex = 'Wanita'
							save_settings()
						elseif settings.player_info.sex == 'Wanita' then
							settings.player_info.sex = 'Pria'
							save_settings()
						end
					end
					imgui.Columns(1)
					imgui.Separator()
					imgui.Columns(3)
					imgui.CenterColumnText("Aksen:")
					imgui.NextColumn()
					if checkbox_accent_enable[0] then
						imgui.CenterColumnText((settings.player_info.accent))
					else 
						imgui.CenterColumnText('Dengan disabilitas')
					end
					imgui.NextColumn()
					if imgui.CenterColumnSmallButton('Mengubah##accent') then
						imgui.OpenPopup(fa.USER_NURSE .. ' Aksen karakter##accent')
					end
					if imgui.BeginPopupModal(fa.USER_NURSE .. ' Aksen karakter##accent', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  ) then
						if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
						...
						imgui.InputText('##accent_input', input_accent, 256) 
						imgui.Separator()
						if imgui.Button(fa.CIRCLE_XMARK .. ' Membatalkan', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
							imgui.CloseCurrentPopup()
						end
					...
					imgui.Separator()
					imgui.Columns(3)
					imgui.CenterColumnText("Staff:")
					imgui.NextColumn()
					imgui.CenterColumnText((settings.player_info.fraction))
					imgui.NextColumn()
					if imgui.CenterColumnSmallButton('Menyegarkan##fraction') then
						check_stats = true
						sampSendChat('/stats')
					end
					imgui.Columns(1)
					imgui.Separator()
					imgui.Columns(3)
					imgui.CenterColumnText("Section staff:")
					imgui.NextColumn()
					imgui.CenterColumnText((settings.player_info.fraction_tag))
					imgui.NextColumn()
					if imgui.CenterColumnSmallButton('Mengubah##fraction_tag') then
						imgui.OpenPopup(fa.BUILDING_SHIELD .. ' Tag organisasi##fraction_tag')
					end
					if imgui.BeginPopupModal(fa.BUILDING_SHIELD .. ' Tag organisasi##fraction_tag', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  ) then
						if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
						imgui.PushItemWidth(405 * settings.general.custom_dpi)
						imgui.InputText('##input_fraction_tag', input_fraction_tag, 256)
						imgui.Separator()
						if imgui.Button(fa.CIRCLE_XMARK .. ' Membatalkan', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
							imgui.CloseCurrentPopup()
						end
					...
					imgui.Separator()
					imgui.Columns(3)
					imgui.CenterColumnText("Rank:")
					imgui.NextColumn()
					imgui.CenterColumnText((settings.player_info.fraction_rank) .. " (" .. settings.player_info.fraction_rank_number .. ")")
					imgui.NextColumn()
					if imgui.CenterColumnSmallButton("Menyegarkan##rank") then
						check_stats = true
						sampSendChat('/stats')
					end
					imgui.Columns(1)
				...
				if imgui.BeginChild('##2', imgui.ImVec2(589 * settings.general.custom_dpi, 52 * settings.general.custom_dpi), true) then
					imgui.CenterText(fa.ROBOT .. ' Asisten')
					imgui.Separator()
					imgui.Columns(2)
					imgui.CenterColumnText(("Asisten Anda yang sangat diperlukan untuk mengotomatiskan beberapa tindakan"))
					imgui.SetColumnWidth(-1, 480 * settings.general.custom_dpi)
					imgui.NextColumn()
					if imgui.CenterColumnSmallButton('Kontrol') then
						imgui.OpenPopup(fa.ROBOT .. ' Asisten untuk memberi pengingat otomatis pada beberapa aktifitas anda')
					end
					if imgui.BeginPopupModal(fa.ROBOT .. ' Asisten untuk mengotomatiskan beberapa tindakan Anda', _, imgui.WindowFlags.NoCollapse  + imgui.WindowFlags.NoResize ) then
						if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
						imgui.BeginChild('##ai', imgui.ImVec2(589 * settings.general.custom_dpi, 360 * settings.general.custom_dpi), true)
						if imgui.Checkbox((' Jika tersisa 2-1 admin dan kondisi kirang kondusif, maka ingatkan melalui chat discord'), checkbox_notify_payday) then
							settings.general.auto_notify_payday = checkbox_notify_payday[0]
							save_settings()
						end
						if imgui.Checkbox((' Pembaruan otomatis daftar admin dalam /admins setiap 3 detik'), checkbox_update_members) then
							settings.general.auto_update_members = checkbox_update_members[0]
							save_settings()
						end
						imgui.EndChild()
						if imgui.Button(fa.CIRCLE_XMARK .. " Menutup", imgui.ImVec2(589 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
							imgui.CloseCurrentPopup()
						end
						imgui.EndPopup()
					end
				...
				if imgui.BeginChild('##3', imgui.ImVec2(589 * settings.general.custom_dpi, 97 * settings.general.custom_dpi), true) then
					imgui.CenterText(fa.SITEMAP .. ' Fungsi tambahan')
					imgui.Separator()
					imgui.Columns(3)
					imgui.CenterColumnText("Autorp rpgun")
					imgui.SameLine(nil, 5) imgui.TextDisabled("[?]")
					if imgui.IsItemHovered() then
						imgui.SetTooltip("Saat menggunakan/menggulir senjata akan ada permainan peran RP dalam obrolan.")
					end
					imgui.SetColumnWidth(-1, 230 * settings.general.custom_dpi)
					imgui.NextColumn()
					if settings.general.rp_gun then
						imgui.CenterColumnText('Aktif')
					else
						imgui.CenterColumnText('Nonaktif')
					end
					imgui.SetColumnWidth(-1, 250 * settings.general.custom_dpi)
					imgui.NextColumn()
					if settings.general.rp_gun then
						if imgui.CenterColumnSmallButton('Disable##rp_gun') then
							settings.general.rp_gun = false
							save_settings()
						end
					else
						if imgui.CenterColumnSmallButton('Enable##rp_gun') then
					...
					imgui.Separator()
					imgui.Columns(3)
					imgui.CenterColumnText("Huruf kapital dan akhiran titik")
					imgui.SameLine(nil, 5) imgui.TextDisabled("[?]")
					if imgui.IsItemHovered() then
						imgui.SetTooltip("Semua pesan Anda akan ditulis dengan huruf kapital dan diakhiri dengan titik.\nBerfungsi dalam obrolan biasa dan beberapa perintah yang sering digunakan:\n/r /rb /j /jb /m /s /b /n /do /vr /fam /al")
					end
					imgui.NextColumn()
					if settings.general.rp_chat then
						imgui.CenterColumnText('Aktif')
					...
					imgui.NextColumn()
					if settings.general.rp_chat then
						if imgui.CenterColumnSmallButton('Disable##rp_chat') then
							settings.general.rp_chat = false
							save_settings()
						end
						else
						if imgui.CenterColumnSmallButton('Enable##rp_chat') then
					...
					imgui.Separator()
					imgui.Columns(3)
					imgui.CenterColumnText("Denied request otomatis beta")
					imgui.SameLine(nil, 5) imgui.TextDisabled("[?]")
					if imgui.IsItemHovered() then
						imgui.SetTooltip("Fungsi hanya untuk pemimpin atau wakilnya!\nMemungkinkan Anda untuk secara otomatis memecat mereka yang meminta PSG\nDengan konfirmasi dari pemain, dengan mengirimkan pesan ke /rb")
					end
					imgui.NextColumn()
					...
					imgui.NextColumn()
					if settings.general.auto_uval then
						if imgui.CenterColumnSmallButton('Disable##auto_uval') then
							settings.general.auto_uval = false
							save_settings()
						end
					else
						if imgui.CenterColumnSmallButton('Enable##auto_uval') then
							if tonumber(settings.player_info.fraction_rank_number) == 9 or tonumber(settings.player_info.fraction_rank_number) == 10 then 
								settings.general.auto_uval = true
								save_settings()
							else
								settings.general.auto_uval = false
								sampAddChatMessage('[Admin Helper] {ffffff}Fungsi ini hanya tersedia untuk pemimpin dan wakilnya!',message_color)
							end
						end
					end
					imgui.Columns(1)
				...
				if imgui.BeginChild('##4', imgui.ImVec2(589 * settings.general.custom_dpi, 28 * settings.general.custom_dpi), true) then
					imgui.Columns(2)
					imgui.Text(" Anda dapat mendukung penulis ulang script (4trays) " .. fa.HAND_HOLDING_DOLLAR)
					imgui.SetColumnWidth(-1, 480 * settings.general.custom_dpi)
					imgui.NextColumn()
					if imgui.CenterColumnSmallButton('About Me') then
						imgui.OpenPopup(fa.SACK_DOLLAR .. ' Dukungan pengembang')
					end
					if imgui.BeginPopupModal(fa.SACK_DOLLAR .. ' Dukungan pengembang', _, imgui.WindowFlags.NoCollapse  + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar) then
						change_dpi()
						if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
						imgui.CenterText('Hubungi 4trays:')
						--imgui.SetCursorPosX(20*settings.general.custom_dpi)
						if imgui.Button(('GitHub'), imgui.ImVec2(imgui.GetMiddleButtonX(2), 25 * settings.general.custom_dpi)) then
							openLink('https://github.com/Donn4try')
						end
						imgui.SameLine()
						if imgui.Button(('Instagram'), imgui.ImVec2(imgui.GetMiddleButtonX(2), 25 * settings.general.custom_dpi)) then
							openLink('https://www.instagram.com/donn.4try?igsh=MXE0eXRrZ2FjYXM0Nw==')
						end
						imgui.Separator()
						if imgui.Button(fa.CIRCLE_XMARK .. ' Menutup', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
							imgui.CloseCurrentPopup()
						end
						imgui.End()
					end
			...
			if imgui.BeginTabItem(fa.RECTANGLE_LIST..' Shortcut Cmd') then 
				if imgui.BeginTabBar('Tabs2') then
					if imgui.BeginTabItem(fa.BARS..' Floating button') then 
						if imgui.BeginChild('##1', imgui.ImVec2(589 * settings.general.custom_dpi, 303 * settings.general.custom_dpi), true) then
							...
							for index, command in ipairs(commands.commands) do
								imgui.Columns(3)
								if command.enable then
									imgui.CenterColumnText('/' .. (command.cmd))
									imgui.NextColumn()
									imgui.CenterColumnText((command.description))
									imgui.NextColumn()
								else
									imgui.CenterColumnTextDisabled('/' .. (command.cmd))
									imgui.NextColumn()
									imgui.CenterColumnTextDisabled((command.description))
									imgui.NextColumn()
								end
								imgui.Text(' ')
								imgui.SameLine()
								if command.enable then
									...
									if imgui.IsItemHovered() then
										imgui.SetTooltip("Menonaktifkan perintah /"..command.cmd)
									end
								else
									...
									if imgui.IsItemHovered() then
										imgui.SetTooltip("Mengaktifkan perintah /"..command.cmd)
									end
								end
								imgui.SameLine()
								if imgui.SmallButton(fa.PEN_TO_SQUARE .. '##'..command.cmd) then
									input_description = imgui.new.char[256]((command.description))
									...
									input_cmd = imgui.new.char[256]((command.cmd))
									change_text = command.text:gsub('&', '\n')		
									input_text = imgui.new.char[8192]((change_text))
									change_waiting = command.waiting
									waiting_slider = imgui.new.float(tonumber(command.waiting))	
									BinderWindow[0] = true
								end
								if imgui.IsItemHovered() then
									imgui.SetTooltip("Perubahan tim /"..command.cmd)
								end
								imgui.SameLine()
								if imgui.SmallButton(fa.TRASH_CAN .. '##'..command.cmd) then
									imgui.OpenPopup(fa.TRIANGLE_EXCLAMATION .. ' Peringatan ##' .. command.cmd)
								end
								if imgui.IsItemHovered() then
									imgui.SetTooltip("Menghapus tim /"..command.cmd)
								end
								if imgui.BeginPopupModal(fa.TRIANGLE_EXCLAMATION .. ' Peringatan ##' .. command.cmd, _, imgui.WindowFlags.NoResize ) then
									if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
									imgui.CenterText('Apakah Anda yakin ingin menghapus perintah? /' .. (command.cmd) .. '?')
									imgui.Separator()
									if imgui.Button(fa.CIRCLE_XMARK .. ' Tidak, batal', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
										imgui.CloseCurrentPopup()
									end
									imgui.SameLine()
									if imgui.Button(fa.TRASH_CAN .. ' Ya, hapus', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
										sampUnregisterChatCommand(command.cmd)
						...
						if imgui.Button(fa.CIRCLE_PLUS .. ' Buat shortcut baru##new_cmd',imgui.ImVec2(imgui.GetMiddleButtonX(1), 0)) then
							...
							input_description = imgui.new.char[256]((new_cmd.description))
							change_arg = new_cmd.arg
							change_bind = new_cmd.bind
							ComboTags[0] = 0
							change_cmd = new_cmd.cmd
							input_cmd = imgui.new.char[256]((new_cmd.cmd))
					...
					if imgui.BeginTabItem(fa.BARS..' Founder access') then 
						if tonumber(settings.player_info.fraction_rank_number) == 9 or tonumber(settings.player_info.fraction_rank_number) == 10 then
							if imgui.BeginChild('##1', imgui.ImVec2(589 * settings.general.custom_dpi, 303 * settings.general.custom_dpi), true) then
								imgui.Columns(3)
								imgui.CenterColumnText("Commands")
								...
								for index, command in ipairs(commands.commands_manage) do
									...
									imgui.SameLine()
									if imgui.SmallButton(fa.TRASH_CAN .. '##'..command.cmd) then
										imgui.OpenPopup(fa.TRIANGLE_EXCLAMATION .. ' Peringatan ##9-10' .. command.cmd)
									end
									if imgui.IsItemHovered() then	
										imgui.SetTooltip("Menghapus tim /"..command.cmd)
									end
									if imgui.BeginPopupModal(fa.TRIANGLE_EXCLAMATION .. ' Peringatan ##9-10' .. command.cmd, _, imgui.WindowFlags.NoResize ) then
										if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
										imgui.CenterText('Apakah Anda yakin ingin menghapus perintah? /' .. (command.cmd) .. '?')
										imgui.Separator()
										if imgui.Button(fa.CIRCLE_XMARK .. ' Tidak, batal', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
							...
							if imgui.Button(fa.CIRCLE_PLUS .. ' Buat shortcut baru##new_cmd_9-10', imgui.ImVec2(imgui.GetMiddleButtonX(1), 0)) then
								binder_create_command_9_10 = true
								local new_cmd = {cmd = '', description = '', text = '', arg = '', enable = true, waiting = '1.500', bind = "{}" }
							...
							imgui.Separator()
							imgui.CenterText("Anda tidak memiliki akses ke perintah ini!")
							imgui.CenterText("Anda harus memiliki peringkat 9 atau 10, tetapi Anda "..settings.player_info.fraction_rank_number.." pangkat!")
							imgui.Separator()
						end
						imgui.EndTabItem() 
					end
					if imgui.BeginTabItem('PC Only (beta)') then 
						if imgui.BeginChild('##999', imgui.ImVec2(589 * settings.general.custom_dpi, 333 * settings.general.custom_dpi), true) then
							if isMonetLoader() then
								imgui.CenterText('Cara membuka menu cepat untuk berinteraksi dengan pemain:')
								if imgui.RadioButtonIntPtr(" Hanya menggunakan perintah /gm [ID pemain]", fastmenu_type, 0) then
									fastmenu_type[0] = 0
									settings.general.mobile_fastmenu_button = false
									save_settings()
									FastMenuButton[0] = false
								end
								if imgui.RadioButtonIntPtr(' Gunakan perintah /gm [ID pemain] atau tombol "Interaksi" di sudut kiri layar', fastmenu_type, 1) then
									fastmenu_type[0] = 1
									settings.general.mobile_fastmenu_button = true
									save_settings()
								end
								imgui.Separator()
								if imgui.Checkbox((' Menampilkan tombol "Stop" (analog dengan /stop)'), checkbox_mobile_stop_button) then
									settings.general.mobile_stop_button = checkbox_mobile_stop_button[0]
									save_settings()
								end
							else
								imgui.CenterText(fa.KEYBOARD .. (' Hotkeys'))
								imgui.CenterText(('Memungkinkan Anda menggunakan kunci (bind) alih-alih memasukkan perintah secara manual'))
								imgui.CenterText(' Fungsionalitas tombol pintas:')
								imgui.SameLine()
								if hotkey_no_errors then
									if settings.general.use_binds then
										...
										if imgui.IsItemHovered() then
											imgui.SetTooltip("Nonaktifkan sistem pengikatan")
										end
										if settings.general.use_binds and hotkey_no_errors then
											imgui.Separator()
											imgui.CenterText('Membuka menu utama helper (command /ahmenu):')
											...
											imgui.Separator()
											imgui.CenterText('Membuka menu cepat untuk berinteraksi dengan pemain (mirip dengan /gm):')
											imgui.CenterText('Bidik pemain dengan RMB dan tekan')
											local width = imgui.GetWindowWidth()
											...
											imgui.Separator()
											imgui.CenterText('Membuka menu kontrol pemutar cepat (mirip dengan /glm):')
											imgui.CenterText('Bidik pemain dengan RMB dan tekan')
											local width = imgui.GetWindowWidth()
											...
											imgui.Separator()
											imgui.CenterText('Jeda pemutaran perintah (analog dengan /stop):')
											local width = imgui.GetWindowWidth()
											...
											imgui.Separator()
											imgui.CenterText('Ikatan yang tersisa tersedia untuk setiap tim RP di bagian penyuntingan tim.')
										end
									else
										...
										if imgui.IsItemHovered() then
											imgui.SetTooltip("Aktifkan sistem pengikatan")
										end
										imgui.Separator()
										imgui.CenterText('Sistem tombol pintas (bind) dinonaktifkan!')
									end
									
								else
									imgui.SameLine()
									imgui.SmallButton(fa.TOGGLE_OFF .. '##enable_binds')
									imgui.CenterText(fa.TRIANGLE_EXCLAMATION .. ' Kesalahan: file perpustakaan hilang!')
								end
								
								if imgui.BeginPopupModal(fa.KEYBOARD .. ' Pengaturan kunci', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  ) then
			...
			if imgui.BeginTabItem(fa.FILE_PEN..' Catatan') then 
			 	imgui.BeginChild('##1', imgui.ImVec2(589 * settings.general.custom_dpi, 330 * settings.general.custom_dpi), true)
				imgui.Columns(2)
				imgui.CenterColumnText("Daftar semua catatan/lembar contekan Anda:")
				...
				for i, note in ipairs(notes.note) do
					imgui.Columns(2)
					imgui.CenterColumnText((note.note_name))
					imgui.NextColumn()
					if imgui.SmallButton(fa.UP_RIGHT_FROM_SQUARE .. '##' .. i) then
						show_note_name = (note.note_name)
						show_note_text = (note.note_text)
						NoteWindow[0] = true
					end
					if imgui.IsItemHovered() then
						imgui.SetTooltip('Catatan terbuka "' .. (note.note_name) .. '"')
					end
					imgui.SameLine()
					if imgui.SmallButton(fa.PEN_TO_SQUARE .. '##' .. i) then
						local note_text = note.note_text:gsub('&','\n')
						input_text_note = imgui.new.char[16384]((note_text))
						input_name_note = imgui.new.char[256]((note.note_name))
						imgui.OpenPopup(fa.PEN_TO_SQUARE .. ' Mengedit catatan' .. '##' .. i)	
					end
					if imgui.IsItemHovered() then
						imgui.SetTooltip('Mengedit catatan "' .. (note.note_name) .. '"')
					end
					if imgui.BeginPopupModal(fa.PEN_TO_SQUARE .. ' Mengedit catatan' .. '##' .. i, _, imgui.WindowFlags.NoCollapse  + imgui.WindowFlags.NoResize ) then
						if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
						if imgui.BeginChild('##9992', imgui.ImVec2(589 * settings.general.custom_dpi, 360 * settings.general.custom_dpi), true) then	
							...
							imgui.InputText('##note_name', input_name_note, 256)
						...
						if imgui.Button(fa.CIRCLE_XMARK .. ' Membatalkan', imgui.ImVec2(imgui.GetMiddleButtonX(2), 0)) then
							imgui.CloseCurrentPopup()
						end
						imgui.SameLine()
						if imgui.Button(fa.FLOPPY_DISK .. ' Simpan catatan', imgui.ImVec2(imgui.GetMiddleButtonX(2), 0)) then
					...
					if imgui.SmallButton(fa.TRASH_CAN .. '##' .. i) then
						imgui.OpenPopup(fa.TRIANGLE_EXCLAMATION .. ' Peringatan ##' .. note.note_name)
					end
					if imgui.IsItemHovered() then
						imgui.SetTooltip('Menghapus catatan "' .. (note.note_name) .. '"')
					end
					if imgui.BeginPopupModal(fa.TRIANGLE_EXCLAMATION .. ' Peringatan ##' .. note.note_name, _, imgui.WindowFlags.NoResize ) then
						if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
						imgui.CenterText('Apakah Anda yakin ingin menghapus catatan tersebut? "' .. (note.note_name) .. '" ?')
						imgui.Separator()
						if imgui.Button(fa.CIRCLE_XMARK .. ' Tidak, batal', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
							imgui.CloseCurrentPopup()
						end
				...
				imgui.EndChild()
				if imgui.Button(fa.CIRCLE_PLUS .. ' Buat catatan baru', imgui.ImVec2(imgui.GetMiddleButtonX(1), 0)) then
					input_name_note = imgui.new.char[256](("Nama"))
					input_text_note = imgui.new.char[16384](("Teks"))
					imgui.OpenPopup(fa.PEN_TO_SQUARE .. ' Buat catatan')	
				end
				if imgui.BeginPopupModal(fa.PEN_TO_SQUARE .. ' Buat catatan', _, imgui.WindowFlags.NoCollapse  + imgui.WindowFlags.NoResize ) then
					if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
					...
					imgui.SameLine()
					if imgui.Button(fa.FLOPPY_DISK .. ' Buat catatan', imgui.ImVec2(imgui.GetMiddleButtonX(2), 0)) then
						local temp = u8:decode(ffi.string(input_text_note))
			...
			if imgui.BeginTabItem(fa.GEAR..' Pengaturan') then 
				imgui.BeginChild('##1', imgui.ImVec2(589 * settings.general.custom_dpi, 145 * settings.general.custom_dpi), true)
				imgui.CenterText(fa.CIRCLE_INFO .. ' Informasi tentang helper')
				imgui.Separator()
				imgui.Text(fa.CIRCLE_USER.." Developer helper: 4trays x MTG MODS")
				imgui.Separator()
				imgui.Text(fa.CIRCLE_INFO.." Versi helper yang terinstal: " .. (thisScript().version))
				imgui.Separator()
				imgui.Text(fa.CODE.." Helper ini dimodifikasi oleh: ")
				imgui.SameLine()
				if imgui.SmallButton('(YT) Icantbenicee') then
					openLink('https://youtube.com/@its.melucy?si=QjKAj2SW_ITP7lzw')
				end
				imgui.Separator()
				imgui.Text(fa.HEADSET.." Dukungan teknis Collaborator:")
				imgui.SameLine()
				if imgui.SmallButton('AsepHytam a.k.a 4trays') then
					openLink('https://donn4try.github.io/mylink/')
				end
				imgui.Separator()
				imgui.Text(fa.GLOBE.." Repo download helper:")
				imgui.SameLine()
				if imgui.SmallButton('Admin Helper V3') then
					openLink('https://github.com/Donn4try/adminmenu')
				end
				imgui.EndChild()
				imgui.BeginChild('##3', imgui.ImVec2(589 * settings.general.custom_dpi, 87 * settings.general.custom_dpi), true)
				imgui.CenterText(fa.PALETTE .. ' Tema warna ui:')
				imgui.Separator()
				if imgui.RadioButtonIntPtr(" Dark Theme ", theme, 0) then	
					theme[0] = 0
				...
				if monet_no_errors then
					if imgui.RadioButtonIntPtr(" MoonMonet Theme ", theme, 1) then
						theme[0] = 1
					...
					if imgui.RadioButtonIntPtr(u8" MoonMonet Theme | "..fa.TRIANGLE_EXCLAMATION .. ' Kesalahan: file perpustakaan hilang!', theme, 1) then
						theme[0] = 0
					end
				end
				imgui.EndChild()
				imgui.BeginChild("##2",imgui.ImVec2(589 * settings.general.custom_dpi, 80 * settings.general.custom_dpi),true)
				imgui.CenterText(fa.MAXIMIZE .. ' Ukuran menu ui:')
				if settings.general.custom_dpi == slider_dpi[0] then
					
				else
					imgui.SameLine()
					if imgui.SmallButton(fa.CIRCLE_ARROW_RIGHT .. ' Terapkan dan simpan') then
						settings.general.custom_dpi = slider_dpi[0]
						save_settings()
						sampAddChatMessage('[Admin Helper] {ffffff}Memulai ulang skrip untuk menerapkan ukuran jendela...', message_color)
						reload_script = true
						thisScript():reload()
					end
				end
				...
				imgui.Separator()
				imgui.CenterText(('Jika menu skrip "mengambang" di layar, ubah ukuran dan pilih nilai'))
				imgui.EndChild()
				imgui.BeginChild("##4",imgui.ImVec2(589 * settings.general.custom_dpi, 35 * settings.general.custom_dpi),true)
				if imgui.Button(fa.POWER_OFF .. " Shut down", imgui.ImVec2(imgui.GetMiddleButtonX(4), 25 * settings.general.custom_dpi)) then
					imgui.OpenPopup(fa.TRIANGLE_EXCLAMATION .. ' Peringatan ##off')
				end
				if imgui.BeginPopupModal(fa.TRIANGLE_EXCLAMATION .. ' Peringatan ##off', _, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar ) then
					if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
					imgui.CenterText('Apakah Anda yakin ingin membongkar (menonaktifkan) helper?')
					imgui.Separator()
					if imgui.Button(fa.CIRCLE_XMARK .. ' Tidak, batal', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
						imgui.CloseCurrentPopup()
					end
					imgui.SameLine()
					if imgui.Button(fa.POWER_OFF .. ' Ya, bongkar', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
						reload_script = true
						play_error_sound()
						sampAddChatMessage('[Admin Helper] {ffffff}Helper telah menangguhkan pekerjaannya hingga login berikutnya ke dalam permainan!', message_color)
						if not isMonetLoader() then 
							sampAddChatMessage('[Admin Helper] {ffffff}Atau gunakan ' .. message_color_hex .. 'CTRL {ffffff}+ ' .. message_color_hex .. 'R {ffffff}untuk menjalankan helper.', message_color)
						end
						thisScript():unload()
					end
					imgui.End()
				...
				imgui.SameLine()
				if imgui.Button(fa.ROTATE_RIGHT .. " Refresh", imgui.ImVec2(imgui.GetMiddleButtonX(4), 25 * settings.general.custom_dpi)) then
					reload_script = true
					thisScript():reload()
				end
				imgui.SameLine()
				if imgui.Button(fa.CLOCK_ROTATE_LEFT .. " Reset pengaturan ", imgui.ImVec2(imgui.GetMiddleButtonX(4), 25 * settings.general.custom_dpi)) then
					imgui.OpenPopup(fa.TRIANGLE_EXCLAMATION .. ' Peringatan ##reset')
				end
				if imgui.BeginPopupModal(fa.TRIANGLE_EXCLAMATION .. ' Peringatan ##reset', _, imgui.WindowFlags.NoResize ) then
					if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
					imgui.CenterText('Apakah Anda yakin ingin mengatur ulang semua pengaturan pembantu?')
					imgui.Separator()
					if imgui.Button(fa.CIRCLE_XMARK .. 'Tidak, batal', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
						imgui.CloseCurrentPopup()
					end
					imgui.SameLine()
					if imgui.Button(fa.CLOCK_ROTATE_LEFT .. ' Ya, atur ulang', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
				...
				imgui.SameLine()
				if imgui.Button(fa.TRASH_CAN .. " Force delete ", imgui.ImVec2(imgui.GetMiddleButtonX(4), 25 * settings.general.custom_dpi)) then
					imgui.OpenPopup(fa.TRIANGLE_EXCLAMATION .. ' Peringatan ##delete')
				end
				if imgui.BeginPopupModal(fa.TRIANGLE_EXCLAMATION .. ' Peringatan ##delete', _, imgui.WindowFlags.NoResize ) then
					if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
					imgui.CenterText('Apakah Anda yakin ingin menghapus Admin Helper?')
					imgui.Separator()
					if imgui.Button(fa.CIRCLE_XMARK .. ' Tidak, batal', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
						imgui.CloseCurrentPopup()
					end
					imgui.SameLine()
					if imgui.Button(fa.TRASH_CAN .. ' Ya, saya ingin menghapus', imgui.ImVec2(200 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
						sampAddChatMessage('[Admin Helper] {ffffff}Helper telah dihapus sepenuhnya dari perangkat Anda!', message_color)
						sampShowDialog(9998, message_color_hex .. "Admin Helper", "Anda telah berhasil menghapus Admin Helper dari perangkat Anda.\nJika penghapusan tersebut disebabkan oleh pengalaman pengguna yang negatif dan Anda mengalami bug atau masalah, beri tahu saya apa yang membuat Anda mengalaminya", "Menutup", '', 0)
						reload_script = true
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(fa.WALKIE_TALKIE .. " Radio deportasi", DeportamentWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize)
		if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
			...
			if imgui.BeginTabBar('TabTags') then
				...
				if imgui.BeginTabItem(fa.BARS..u8' Ваши кастомные теги ') then 
					...
					if imgui.BeginPopupModal(fa.TAG .. u8' Добавление нового тега##1', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize) then
						...
						imgui.SameLine()
						if imgui.Button(fa.FLOPPY_DISK .. ' Menyimpan', imgui.ImVec2( imgui.GetMiddleButtonX(2), 25 * settings.general.custom_dpi)) then
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.SetNextWindowSize(imgui.ImVec2(600 * settings.general.custom_dpi, 425	* settings.general.custom_dpi), imgui.Cond.FirstUseEver)
		imgui.Begin(fa.PEN_TO_SQUARE .. ' Mengedit slash command' .. change_cmd, BinderWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  )
		if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
		if imgui.BeginChild('##binder_edit', imgui.ImVec2(589 * settings.general.custom_dpi, 361 * settings.general.custom_dpi), true) then
			imgui.CenterText(fa.FILE_LINES .. ' Deskripsi perintah:')
			imgui.PushItemWidth(579 * settings.general.custom_dpi)
			imgui.InputText("##input_description", input_description, 256)
			imgui.Separator()
			imgui.CenterText(fa.TERMINAL .. ' Perintah untuk digunakan dalam obrolan (tanpa /):')
			imgui.PushItemWidth(579 * settings.general.custom_dpi)
			imgui.InputText("##input_cmd", input_cmd, 256)
			imgui.Separator()
			imgui.CenterText(fa.CODE .. ' Argumen yang diterima perintah:')
	    	imgui.Combo('',ComboTags, ImItems, #item_list)
	 	    imgui.Separator()
	        imgui.CenterText(fa.FILE_WORD .. ' Perintah teks bind:')
		...
		imgui.EndChild() end
		if imgui.Button(fa.CIRCLE_XMARK .. ' Membatalkan', imgui.ImVec2(imgui.GetMiddleButtonX(5), 0)) then
			BinderWindow[0] = false
		end
		imgui.SameLine()
		if imgui.Button(fa.CLOCK .. ' Menunda',imgui.ImVec2(imgui.GetMiddleButtonX(5), 0)) then
			imgui.OpenPopup(fa.CLOCK .. ' Penundaan (dalam detik) ')
		end
		if imgui.BeginPopupModal(fa.CLOCK .. ' Penundaan (dalam detik) ', _, imgui.WindowFlags.NoResize ) then
			if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
			imgui.PushItemWidth(200 * settings.general.custom_dpi)
			imgui.SliderFloat('##waiting', waiting_slider, 0.3, 5)
			imgui.Separator()
			if imgui.Button(fa.CIRCLE_XMARK .. ' Membatalkan', imgui.ImVec2(imgui.GetMiddleButtonX(2), 0)) then
			...
			imgui.SameLine()
			if imgui.Button(fa.FLOPPY_DISK .. ' Menyimpan', imgui.ImVec2(imgui.GetMiddleButtonX(2), 0)) then
				imgui.CloseCurrentPopup()
			end
			imgui.End()
		end
		imgui.SameLine()
		if imgui.Button(fa.TAGS .. '  Tag', imgui.ImVec2(imgui.GetMiddleButtonX(5), 0)) then
			imgui.OpenPopup(fa.TAGS .. ' Tag untuk digunakan dalam binder')
		end
		if imgui.BeginPopupModal(fa.TAGS .. ' Tag untuk digunakan dalam binderе', _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize ) then
			if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
			imgui.Text(u8(binder_tags_text))
			imgui.Separator()
			if imgui.Button(fa.CIRCLE_XMARK .. ' Menutup', imgui.ImVec2(imgui.GetMiddleButtonX(1), 0)) then
				imgui.CloseCurrentPopup()
			end
			imgui.End()
		end
		imgui.SameLine()
		if imgui.Button(fa.KEYBOARD .. ' Bind (untuk PC)', imgui.ImVec2(imgui.GetMiddleButtonX(5), 0)) then
			if isMonetLoader() then
				sampAddChatMessage('[Admin Helper] {ffffff}Fitur ini hanya tersedia di PC!', message_color)
			else
				if hotkey_no_errors then
					if ComboTags[0] == 0 then
						if settings.general.use_binds then 
							imgui.OpenPopup(fa.KEYBOARD .. ' Mengikat untuk tim /' .. change_cmd)
						else
							sampAddChatMessage('[Admin Helper] {ffffff}Pertama, aktifkan hotkey di Perintah dan Pemutaran - Fungsi Tambahan', message_color)
						end
					else
						sampAddChatMessage('[Admin Helper] {ffffff}Fungsi ini hanya tersedia jika perintahnya adalah "Tidak ada argumen""', message_color)
					end
				else
					sampAddChatMessage('[Admin Helper] {ffffff}Fungsi ini tidak tersedia, alasannya adalah file pustaka mimgui_hotkeys hilang!', message_color)
				end
			end
		end
		if imgui.BeginPopupModal(fa.KEYBOARD .. ' Mengikat untuk tim /' .. change_cmd, _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize) then
			if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
			local hotkeyObject = hotkeys[change_cmd .. "HotKey"]
			if hotkeyObject then
				imgui.CenterText(('Kunci aktivasi ikatan:'))
				local calc
				...
				if hotkeyObject:ShowHotKey() then
					...
					sampAddChatMessage('[Admin Helper] {ffffff}Tombol pintas untuk perintah telah dibuat ' .. message_color_hex .. '/' .. change_cmd .. ' {ffffff}pada kunci '  .. message_color_hex .. getNameKeysFrom(change_bind), message_color)
				end
			else
				...
				--sampAddChatMessage('[Admin Helper] {ffffff}Tombol pintas baru telah dibuat, antarmuka perlu dibuat ulang. Buka kembali menu ini.', message_color)
				--imgui.CloseCurrentPopup()
			end
			imgui.Separator()
		...
		imgui.SameLine()
		if imgui.Button(fa.FLOPPY_DISK .. ' Menyimpan', imgui.ImVec2(imgui.GetMiddleButtonX(5), 0)) then	
			if ffi.string(input_cmd):find('%W') or ffi.string(input_cmd) == '' or ffi.string(input_description) == '' or ffi.string(input_text) == '' then
				imgui.OpenPopup(fa.TRIANGLE_EXCLAMATION .. ' Terjadi kesalahan saat menyimpan perintah!')
			else
				local new_arg = ''
				if ComboTags[0] == 0 then
					new_arg = ''
				...
				for _, command in ipairs(temp_array) do
					if command.cmd == change_cmd and command.arg == change_arg and command.text:gsub('&', '\n') == change_text then
						...
						save_commands()
						if command.arg == '' then
							sampAddChatMessage('[Admin Helper] {ffffff}Tim ' .. message_color_hex .. '/' .. new_command .. ' {ffffff}berhasil disimpan!', message_color)
						elseif command.arg == '{arg}' then
							sampAddChatMessage('[Admin Helper] {ffffff}Tim ' .. message_color_hex .. '/' .. new_command .. ' [argumen] {ffffff}berhasil disimpan!', message_color)
						elseif command.arg == '{arg_id}' then
							sampAddChatMessage('[Admin Helper] {ffffff}Perintah ' .. message_color_hex .. '/' .. new_command .. ' [ID pemain] {ffffff}berhasil disimpan!', message_color)
						elseif command.arg == '{arg_id} {arg2}' then
							sampAddChatMessage('[Admin Helper] {ffffff}Perintah ' .. message_color_hex .. '/' .. new_command .. ' [ID pemain] [argumen] {ffffff}berhasil disimpan! Perintah ' .. message_color_hex .. '/' .. new_command .. ' [ID pemain] [argumen] {ffffff}berhasil disimpan!', message_color)
						elseif command.arg == '{arg_id} {arg2} {arg3}' then
							sampAddChatMessage('[Admin Helper] {ffffff}Perintah ' .. message_color_hex .. '/' .. new_command .. ' [ID pemain] [argumen] [argumen] {ffffff}berhasil disimpan!', message_color)
						end
		...
		if imgui.BeginPopupModal(fa.TRIANGLE_EXCLAMATION .. ' Terjadi kesalahan saat menyimpan perintah!', _, imgui.WindowFlags.AlwaysAutoResize ) then
			if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
			if ffi.string(input_cmd):find('%W') then
				imgui.BulletText(" Anda hanya dapat menggunakan huruf dan/atau angka bahasa Inggris dalam perintah!")
			elseif ffi.string(input_cmd) == '' then
				imgui.BulletText(" Tim tidak boleh kosong!")
			end
			if ffi.string(input_description) == '' then
				imgui.BulletText(" Deskripsi perintah tidak boleh kosong!")
			end
			if ffi.string(input_text) == '' then
				imgui.BulletText(" Perintah bind tidak boleh kosong!")
			end
			imgui.Separator()
			if imgui.Button(fa.CIRCLE_XMARK .. ' Menutup', imgui.ImVec2(300 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
				imgui.CloseCurrentPopup()
			end
			imgui.End()
		end	
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(fa.ROBOT .. " Admin Helper##fast_menu_button", FastMenuButton, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.NoTitleBar  )
		change_dpi()
		if imgui.Button(fa.SITEMAP .. ' Player Menu ') then
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(fa.HOSPITAL.." Player list##fast_menu_players", FastMenuPlayers, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize  )
		change_dpi()
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(fa.ROBOT .. " Admin Helper##rank", GiveRankMenu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize)
		if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
		imgui.CenterText('Pilih rank untuk '.. sampGetPlayerNickname(player_id) .. ':')
		imgui.PushItemWidth(250 * settings.general.custom_dpi)
		imgui.SliderInt('', giverank, 1, 10)
		imgui.Separator()
		local text
		if isMonetLoader() then
			text = " Berikan rank"
		else
			text = " Berikan rank [Enter]"
		end
		if imgui.Button(fa.USER_NURSE .. (text) , imgui.ImVec2(imgui.GetMiddleButtonX(1), 0)) then
			give_rank()
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(fa.ROBOT .. " Admin Helper##CommandStopWindow", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize )
		if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
		if isMonetLoader() and isActiveCommand then
			if imgui.Button(fa.CIRCLE_STOP..' Berhenti bermain ') then
				command_stop = true 
				CommandStopWindow[0] = false
			end
		else
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY - 50 * settings.general.custom_dpi), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(fa.ROBOT .. " Admin Helper##CommandPauseWindow", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize )
		if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
		if command_pause then
			if imgui.Button(fa.CIRCLE_ARROW_RIGHT .. ' Melanjutkan ', imgui.ImVec2(150 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
				command_pause = false
				CommandPauseWindow[0] = false
			end
			imgui.SameLine()
			if imgui.Button(fa.CIRCLE_XMARK .. ' Penuh STOP ', imgui.ImVec2(150 * settings.general.custom_dpi, 25 * settings.general.custom_dpi)) then
				command_stop = true 
				command_pause = false
				CommandPauseWindow[0] = false
			end
...
imgui.OnFrame(
    ...
    function(player)
		if player_id ~= nil and isParamSampID(player_id) then
			imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
			imgui.Begin(fa.PERSON_CIRCLE_CHECK..' Melakukan wawancara dengan pemain ' .. sampGetPlayerNickname(player_id), SobesMenu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize)
			if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
			if imgui.BeginChild('sobes1', imgui.ImVec2(240 * settings.general.custom_dpi, 182 * settings.general.custom_dpi), true) then
			imgui.CenterColumnText(fa.BOOKMARK .. " Utama")
			imgui.Separator()
			if imgui.Button(fa.PLAY .. " Mulai wawancara", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
				lua_thread.create(function()
					sampSendChat("Halo saya " .. settings.player_info.name_surname .. " - " .. settings.player_info.fraction_rank .. ' ' .. settings.player_info.fraction_tag)
					wait(2000)
					sampSendChat("Apakah Anda datang kepada kami untuk wawancara?")
				end)
			end
			if imgui.Button(fa.PASSPORT .. " Meminta dokumen", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
				lua_thread.create(function()
					sampSendChat("Oke, berikan saya semua dokumen Anda untuk verifikasi.")
					wait(2000)
					sampSendChat("Saya butuh Paspor, Kartu Medis, dan Lisensi Anda.")
					wait(2000)
					sampSendChat("/n " .. sampGetPlayerNickname(player_id) .. ", menggunakan /showpass [ID] , /showmc [ID] , /showlic [ID]")
					wait(2000)
					sampSendChat("/n Tentu saja dengan permainan peran RP!")
				end)
			end
			if imgui.Button(fa.USER .. " Ceritakan tentang dirimu", imgui.ImVec2(-1, 25 * settings.general.custom_dpi)) then
				sampSendChat("Ceritakan sedikit tentang diri Anda.")
			end
			...
			sampAddChatMessage('[Admin Helper] {ffffff}Прозиошла ошибка, ID игрока недействителен!', message_color)
			SobesMenu[0] = false
		end
    end
)
...
function main()
	...
	welcome_message()
	sampRegisterChatCommand("gm", show_fast_menu)
	initialize_commands()

	if settings.player_info.name_surname == '' or settings.player_info.fraction == 'Неизвестно' then
		sampAddChatMessage('[Admin Helper] {ffffff}Пытаюсь получить ваш /stats поскольку остуствуют данные про вас!', message_color)
		check_stats = true
		sampSendChat('/stats')
	end
	...
	while true do
		...
		if ((os.date("%M", os.time()) == "55" and os.date("%S", os.time()) == "00") or (os.date("%M", os.time()) == "25" and os.date("%S", os.time()) == "00")) then
			if sampGetPlayerColor(tagReplacements.my_id()) == 368966908 then
				sampAddChatMessage('[Admin Helper] {ffffff}Через 5 минут будет PAYDAY. Наденьте форму чтобы не пропустить зарплату!', message_color)
				wait(1000)
			end
		end
...
function onScriptTerminate(script, game_quit)
    if script == thisScript() and not game_quit and not reload_script then
		sampAddChatMessage('[Admin Helper] {ffffff}Произошла неизвестная ошибка, хелпер приостановил свою работу!', message_color)
		if not isMonetLoader() then 
			sampAddChatMessage('[Admin Helper] {ffffff}Используйте ' .. message_color_hex .. 'CTRL {ffffff}+ ' .. message_color_hex .. 'R {ffffff}чтобы перезапустить хелпер.', message_color)
		end
		play_error_sound()
    end
end