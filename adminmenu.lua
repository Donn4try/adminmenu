...
local commands = {
	commands = {
		{ cmd = 'stats' , description = 'Player Statistic' , text = '/astats {arg_id}', arg = '{arg_id}' , enable = true , waiting = '1.200', deleted = false },
		{ cmd = 'ahealth' , description = 'Player Health' , text = '/ahealth {arg_id}', arg = '{arg_id}' , enable = true, waiting = '1.200', deleted = false   },
		{ cmd = 'adamagelogs' , description = 'Player Damagelogs' ,  text = '/adamagelogs {arg_id}', arg = '{arg_id}' , enable = true, waiting = '1.200', deleted = false   },
		{ cmd = 'freeze' , description = 'Freeze Player' , text = '/freeze {arg_id}', arg = '{arg_id}' , enable = true, waiting = '1.200', deleted = false   },
		{ cmd = 'unfreeze' , description = 'Unfreeze Player' ,  text = '/unfreeze {arg_id}' , arg = '{arg_id}' , arg = '{arg_id}' , enable = true , waiting = '1.500', deleted = false  },		
		{ cmd = 'slap' , description = 'Slap Player' ,  text = '/slap {arg_id}' , arg = '{arg_id}', enable = true, waiting = '1.200', deleted = false} , 
		{ cmd = 'mz' , description = 'Revive Player' ,  text = '/arv {arg_id}' , arg = '{arg_id}' , enable = true , waiting = '1.200' , deleted = false },
					...
					for line_index, line in ipairs(lines) do
						if command_stop then 
							...
							elseif line == "{pause}" then
								sampAddChatMessage('[Admin Helper] {ffffff}Команда /' .. chat_cmd .. ' поставлена на паузу!', message_color)
								command_pause = true
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(fa.HOSPITAL .. " Admin Helper##main", MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize )
		change_dpi()
		if imgui.BeginTabBar('пон') then	
			if imgui.BeginTabItem(fa.ID_CARD..' Menu utama') then
				...
				if imgui.BeginChild('##3', imgui.ImVec2(589 * settings.general.custom_dpi, 97 * settings.general.custom_dpi), true) then
					imgui.CenterText(fa.SITEMAP .. ' Fungsi tambahan helper')
					imgui.Separator()
					imgui.Columns(3)
			...
			if imgui.BeginTabItem(fa.RECTANGLE_LIST..' Team dan Autorp') then 
				if imgui.BeginTabBar('Tabs2') then
					if imgui.BeginTabItem(fa.BARS..' Cmd') then 
						...
						if imgui.Button(fa.CIRCLE_PLUS .. ' Buat shorcut baru##new_cmd',imgui.ImVec2(imgui.GetMiddleButtonX(1), 0)) then
					...
					if imgui.BeginTabItem(fa.BARS..' HighStaff commands') then 
						if tonumber(settings.player_info.fraction_rank_number) == 9 or tonumber(settings.player_info.fraction_rank_number) == 10 then
							if imgui.BeginChild('##1', imgui.ImVec2(589 * settings.general.custom_dpi, 303 * settings.general.custom_dpi), true) then
								imgui.Columns(3)
								imgui.CenterColumnText("Commands")
			...
			if imgui.BeginTabItem(fa.GEAR..' Pengaturan') then 
				...
				imgui.CenterText(fa.CIRCLE_INFO .. ' Informasi tambahan tentang helper')
				imgui.Separator()
				...
				imgui.CenterText(fa.PALETTE .. ' Tema warna pembantu:')
				imgui.Separator()
				...
				imgui.CenterText(fa.MAXIMIZE .. ' Ukuran menu skrip:')
				...
				if imgui.Button(fa.POWER_OFF .. " Penutupan ", imgui.ImVec2(imgui.GetMiddleButtonX(4), 25 * settings.general.custom_dpi)) then
				...
				imgui.SameLine()
				if imgui.Button(fa.ROTATE_RIGHT .. " Menyalakan ulang ", imgui.ImVec2(imgui.GetMiddleButtonX(4), 25 * settings.general.custom_dpi)) then
					reload_script = true
					thisScript():reload()
				end
				imgui.SameLine()
				if imgui.Button(fa.CLOCK_ROTATE_LEFT .. " Atur ulang pengaturan ", imgui.ImVec2(imgui.GetMiddleButtonX(4), 25 * settings.general.custom_dpi)) then
				...
				imgui.SameLine()
				if imgui.Button(fa.TRASH_CAN .. " Pemindahan ", imgui.ImVec2(imgui.GetMiddleButtonX(4), 25 * settings.general.custom_dpi)) then
			...
			if ffi.string(input_cmd):find('%W') or ffi.string(input_cmd) == '' or ffi.string(input_description) == '' or ffi.string(input_text) == '' then
				...
				for _, command in ipairs(temp_array) do
					if command.cmd == change_cmd and command.arg == change_arg and command.text:gsub('&', '\n') == change_text then
						...
						elseif command.arg == '{arg_id}' then
							sampAddChatMessage('[Admin Helper] {ffffff}Команда ' .. message_color_hex .. '/' .. new_command .. ' [ID игрока] {ffffff}успешно сохранена!', message_color)
						elseif command.arg == '{arg_id} {arg2}' then
							sampAddChatMessage('[Admin Helper] {ffffff}Команда ' .. message_color_hex .. '/' .. new_command .. ' [ID игрока] [аргумент] {ffffff}успешно сохранена!', message_color)
						elseif command.arg == '{arg_id} {arg2} {arg3}' then
							sampAddChatMessage('[Admin Helper] {ffffff}Команда ' .. message_color_hex .. '/' .. new_command .. ' [ID игрока] [аргумент] [аргумент] {ffffff}успешно сохранена!', message_color)
						end
...
imgui.OnFrame(
    ...
    function(player)
		...
		change_dpi()
		if imgui.Button(fa.IMAGE_PORTRAIT..' Player Menu ') then
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(fa.BUILDING_SHIELD.." Admin Helper##rank", GiveRankMenu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize)
		if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
...
imgui.OnFrame(
    ...
    function(player)
		...
		imgui.Begin(fa.BUILDING_SHIELD .. " Admin Helper##CommandStopWindow", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize )
		if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
...
imgui.OnFrame(
    ...
    function(player)
		imgui.SetNextWindowPos(imgui.ImVec2(sizeX / 2, sizeY - 50 * settings.general.custom_dpi), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
		imgui.Begin(fa.BUILDING_SHIELD.." Admin Helper##CommandPauseWindow", _, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize  + imgui.WindowFlags.NoScrollbar + imgui.WindowFlags.AlwaysAutoResize )
		if not isMonetLoader() then imgui.SetWindowFontScale(settings.general.custom_dpi) end
